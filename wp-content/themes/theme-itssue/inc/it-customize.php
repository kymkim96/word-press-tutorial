<?php
/**
 * functions.php - 기능 구현을 담당하는 파일입니다.
 *
...
function it_theme_customize_register($wp_customize)
{
    ...
    $wp_customize->get_control('header_textcolor')->priority
        = 50;
    $wp_customize->get_control('header-image')->section
        = 'it_customize_header';
    ...
    $wp_customize->add_control(
        new WP_Customize_Control($wp_customize, 'it_customize_naver_meta',
            array(
                'label' => '소유 확인 코드',
                'description' => '네이버 웹마스터도구에서 제공한 소유 확인 코드를 입력해주세요.',
                'type' => 'text',
  ...
  ';
    ...
    for ($i = 1; $i <= 7; $i++) {
        ...
        $wp_customize->add_control(
            new IT_Customize_Control($wp_customize, $prefix_setting . 'category',
                array(
    ...
    $wp_customize->add_control(
        new IT_Customize_Control($wp_customize, 'it_customize_front_sns',
            array(
                'label' => 'SNS 숏코드',
                'description' => 'SNS 타임라인 출력을 위한 숏코드를 입력합니다.',
                'section' => 'it_customize_front_page'
            ))
    );
}
